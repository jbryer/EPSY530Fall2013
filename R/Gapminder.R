...
ggplot(worldData3, aes(x=GDP, y=Life.Expectancy, label=Country)) + 
	geom_point(aes(size=Population, colour=Region), stat='identity', alpha=.6) + 
	geom_text(hjust=-.2, vjust=.5, size=2) + 
	scale_size_continuous('Population (Millions)', range=c(1,20)) + 
	xlab('Gross Domestic Product (billions)') + ylab('Life Expectancy at birth (years)')

ggplot(worldData3, aes(x=GDP.log, y=Life.Expectancy, label=Country)) +
	geom_point(aes(size=Population, colour=Region), stat='identity', alpha=.6) + 
	geom_text(hjust=-.2, vjust=.5, size=2) + 
	scale_size_continuous('Population (Millions)', range=c(1,20)) + 
	xlab('Gross Domestic Product (log scale)') + ylab('Life Expectancy at birth (years)')

worldData4 = worldData3[which(worldData3$GDP < 2000),]
ggplot(worldData4, aes(x=GDP, y=Life.Expectancy, label=Country)) + 
	geom_point(aes(size=Population, colour=Region), stat='identity', alpha=.6) + 
	geom_text(hjust=-.2, vjust=.5, size=2) + 
	scale_size_continuous('Population (Millions)', range=c(1,20)) + 
	xlab('Gross Domestic Product (billions)') + ylab('Life Expectancy at birth (years)')
